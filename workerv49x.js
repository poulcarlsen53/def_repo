...
function createIdleWatchdog(log, onTimeout, idleMs = 300000, checkMs = 30000) {
...
const TCP_CONNECTION_TIMEOUT_MS = 10000;
...
async function handleTCPOutBound(ctx, remoteSocketWapper, addressRemote, portRemote, rawClientData, webSocket, VLResponseHeader, log, onActivity = () => {}) {
  ...
  // A7: cbSkips tracks circuit-breaker skips separately so they don't eat real retry slots.
  // Real connection failures still consume i (loop increment). Max skips = total attempt budget.
  let cbSkips = 0;
  const MAX_CB_SKIPS = MAX_RETRIES + 1;
  for (let i = 0; i <= MAX_RETRIES; i++) {
    ...
    if (!shouldTryEndpoint(targetAddress, targetPort)) {
      cbSkips++;
      log(`Circuit breaker open for ${targetAddress}:${targetPort}, skip ${cbSkips}/${MAX_CB_SKIPS}`);
      if (cbSkips < MAX_CB_SKIPS) {
        await chooseNextTarget();
        i--; // don't consume retry budget for a skipped endpoint
        continue;
      }
      remoteSocketWapper.connecting = false;
      log(`All endpoints circuit-broken after ${cbSkips} skips, giving up`);
...
async function waitForWebSocketDrain(webSocket, maxBuffer = 1024 * 1024, resumeBuffer = 512 * 1024, timeoutMs = 5000) {
  if (typeof webSocket.bufferedAmount !== "number") {
    return; // bufferedAmount not supported in CF server-side WS — nothing to drain
  }
...
async function remoteSocketToWS(remoteSocket, webSocket, VLResponseHeader, log, onActivity = () => {}) {
  ...
  const writableStream = new WritableStream({
    async write(chunk, controller) {
      ...
      if (typeof webSocket.bufferedAmount === "number") await waitForWebSocketDrain(webSocket);
      if (vlHeader) {
...
async function parseTrHeader(ctx, buffer) {
  ...
  const password = decoder.decode(buffer.slice(0, crLfIndex));
  ...
  switch (atype) {
    ...
    case 3:
      ...
      address = decoder.decode(socks5DataBuffer.slice(addressIndex, addressIndex + addressLength));
      break;
    case 4: {
      addressLength = 16;
      if (addressIndex + addressLength > socks5DataBuffer.byteLength) {
      ...
      // D33: offset DataView into existing buffer — no slice allocation
      const dataView = new DataView(socks5DataBuffer, addressIndex, addressLength);
      const ipv6 = [];
      for (let i = 0; i < 8; i++) {
        ipv6.push(dataView.getUint16(i * 2).toString(16));
      }
...
var worker_default = {
  ...
  async handleFetch(request, env, context, releaseConnection = () => {}) {
    try {
      ...
      const dataset = await getDataset(request, env, context);
      // Shallow copy so mutations below don't corrupt SETTINGS_CACHE.data
